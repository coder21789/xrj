 ...
 */
module.exports = function getRouter(app, options) {
  ...
  if (!fs.existsSync(root)) {
    logger.error('error : can\'t find route path ' + root);
  ...
  _ls(root).forEach((filePath) => {
    ...
    try {
      ...
      logger.error(`error: require ${filePath} error ${err}`);
      return;
    }
  ...
  app.context.__defineGetter__("bindDefault", () => {
    try {
      ...
      return new Promise((resolve) => {
        logger.error(`Cannot find default controller '${defaultCtrlPath}'`)
        resolve();
      })
    }
  });

  return async function getRouter(ctx, next) {
...
function getRoute(exportFuncs, cb, ctrlpath, curCtrlname) {
  ...
  if (ctrlpath.length > 3) {
    logger.error(`嵌套路由对象层级不能超过3级：${totalCtrlname.join('/')}`);
    return;
  }
 ...
 * @param {Obejct} options get router配置
 */
function _setRoute(Router, config, options) {
  ...
  method.forEach((_method) => {
    ...
    paths.forEach((pathItem) => {
      logger.info(_method + ':' + config.domain + pathItem);
