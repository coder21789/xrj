...
module.exports = function graceMongo(app, options) {
  ...
  if (!fs.existsSync(root)) {
    logger.error('error : can\'t find mongo path ' + root);
  ...
  db.on('error', function(err) {
    logger.error(`error: connect ${connect} ${err}`);
  });
  db.once('open', function() {
    logger.info(`connect ${connect} success!`);
  });
  ...
  _ls(root).forEach(function(filePath) {
    ...
    // 创建schema
    let _schema = new mongoose.Schema(mod.schema[0]);
