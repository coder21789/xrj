...
module.exports = function proxy(app, api, config, options) {
  ...
  return async function(ctx, next) {
    ...
    Object.assign(ctx, {
      proxy: function(opt, config) {
        ...
        // 装载头信息的容器
        let headerObj = config.hasOwnProperty("headers") ? config.headers : false; 
        let params = config.hasOwnProperty("params") ?config.params : false;
        ...
        let reqs = reqsParam.map((proxyName) => {
          ...
          //header 设置后test realReq.headers['api-version']='1.7.0';

          headerObj && (headerObj[proxyName] = realReq.headers);
          let header =  headerObj || realReq.headers;
          let uri = params ? realReq.uri + getStr(params) : realReq.uri;

          // 请求request的最终配置
          let requestOpt = Object.assign({}, options, {
            uri: uri,
            method: realReq.method,
            headers: header,//realReq.headers
            timeout: undefined,
            gzip: true,
            encoding: null
          }, config.conf);
  ...
  function setResCookies(ctx, headers) {
    ...
    ctx.res._headers['set-cookie'] = !!ctx.res._headers['set-cookie'] ? ctx.res._headers['set-cookie'].concat(cookies) : [].concat(cookies) ;
