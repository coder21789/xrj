  ...
  function _createReq(resolve) {
    ...
    return Request(opt, (err, httpResponse, data) => {
      ...
      if (err) {
        ...
        logger.error('proxy error : ' + opt.uri, err);
      ...
      // 没有报错，且有正常的返回数据
      if (!err && data) {
        logger.info('proxy success : ' + opt.uri, info);
      ...
      // 根据重试配置进行重试
      if (retryNum > 0) {
        logger.info(`proxy retry: Request ${opt.uri} no response, retry ${retryNum} times!`, info);
        retryNum--;
        return _createReq(resolve)
      } else {
        logger.error(`proxy error: Request ${opt.uri} no response!`, info);
